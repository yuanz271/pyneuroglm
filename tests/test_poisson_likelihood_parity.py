...
class TestMAPRegression:
    ...
    def test_map_parity_with_matlab(self):
        """
        Compare Python MAP against MATLAB exampleMAP.mat.

        MATLAB quasi-newton stops at grad_norm ~3.86; Python trust-ncg reaches ~1e-7.
        Both reach same objective (~669.67), so we test objective parity and weight
        correlation rather than exact weight equality.
        """
        import scipy.sparse
        ...
        wml_matlab = to_dense(mat["wml"]).ravel()
        nlogli_matlab = float(to_dense(mat["nlogli"]).ravel()[0])
        grad_norm_matlab = float(to_dense(mat["grad_norm"]).ravel()[0])
        ...
        L_py, dL_py, _ = poisson_loglik(w_py, X, y, exp_nonlinearity, inds)
        P_py, dP_py, _ = gaussian_zero_mean_inv(w_py, Cinv)
        obj_py = -(L_py + P_py)
        grad_py = np.linalg.norm(-(dL_py + dP_py))

        print("\n=== MAP MATLAB Parity Test ===")
        print(f"MATLAB objective:  {nlogli_matlab:.8f}")
        print(f"Python objective:  {obj_py:.8f}")
        print(f"Objective diff:    {abs(obj_py - nlogli_matlab):.2e}")
        print(f"MATLAB grad_norm:  {grad_norm_matlab:.4f}")
        print(f"Python grad_norm:  {grad_py:.2e}")
